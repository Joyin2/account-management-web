/**
 * Firestore Database Reset Script (Authenticated Version)
 * 
 * This script authenticates with Firebase Auth first, then performs
 * the database reset with user permissions.
 * 
...
async function authenticateUser() {
  ...
  try {
    ...
    console.log('Please sign in with an admin account to perform database reset.');
    console.log('='.repeat(80));
    
    const email = await askQuestion(rl, 'Email: ');
    const password = await askQuestion(rl, 'Password: ', true);
...
async function analyzeDatabase() {
  ...
  for (const collectionName of COLLECTIONS_TO_RESET) {
    try {
      ...
      const snapshot = await getDocs(query(collectionRef, limit(1000)));
      const docCount = snapshot.size;
      
      analysis.accessibleCollections.push(collectionName);
      ...
      if (docCount > 0) {
        ...
        logger.info(`Collection '${collectionName}': empty`);
        analysis.collections[collectionName] = 0;
      }
    } catch (error) {
      logger.warning(`Could not access collection '${collectionName}': ${error.message}`);
      analysis.collections[collectionName] = 'inaccessible';
    }
  }

  logger.info(`Accessible collections: ${analysis.accessibleCollections.length}`);
...
async function deleteCollectionDocuments(collectionName) {
  try {
    ...
    for (let i = 0; i < docs.length; i += batchSize) {
      ...
      logger.info(`Deleted ${deletedCount}/${docs.length} documents from '${collectionName}'`);
    }
...
async function confirmOperation(analysis) {
  ...
  try {
    ...
    console.log('\nDatabase Analysis:');
    console.log(`- Accessible collections: ${analysis.accessibleCollections.length}`);
    ...
    COLLECTIONS_TO_RESET.forEach(name => {
      const count = analysis.collections[name];
      if (count === 'inaccessible') {
        console.log(`  - ${name}: inaccessible (may require different permissions)`);
      } else {
...
async function main() {
  ...
  try {
    console.log('\n' + '='.repeat(80));
    console.log('FIRESTORE DATABASE RESET OPERATION STARTED (AUTHENTICATED)');
    console.log('='.repeat(80));
    ...
    // Check if we have any accessible collections
    if (analysis.accessibleCollections.length === 0) {
      logger.error('No collections are accessible with current user permissions');
      logger.error('Please ensure the authenticated user has proper Firestore permissions');
      process.exit(1);
    }
    ...
    if (shouldBackup) {
      ...
      for (const collectionName of analysis.accessibleCollections) {
    ...
    logger.success('All accessible collections recreated as empty');
    ...
    if (finalAnalysis.totalDocuments === 0) {
      ...
      logger.success('✅ All accessible collections are now empty');
    } else {
    ...
    if (user && auth.currentUser) {
      try {
        ...
        logger.warning(`Failed to sign out user: ${error.message}`);
      }
    }
  }
}
...
process.on('SIGINT', async () => {
  ...
  if (auth && auth.currentUser) {
    try {
      ...
      // Ignore sign out errors during interruption
    }
  }
  process.exit(1);
});
...
process.on('SIGTERM', async () => {
  ...
  if (auth && auth.currentUser) {
    try {
      ...
      // Ignore sign out errors during termination
    }
  }
  process.exit(1);
});
